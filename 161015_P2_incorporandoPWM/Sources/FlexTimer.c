...
volatile uint32* FTMx_CONF[3]={&FTM0_C0SC, &FTM1_C0SC, &FTM2_C0SC};
...
void FTM_C0SC(FTMType FTM, uint32 mask){
	*FTMx_C0SC[FTM] |= mask;
}
...
void FTM_updateCHValue(FTMType FTM,sint16 channelValue)
{
	/**Assigns a new value for the duty cycle*/
	*FTMx_C0V[FTM] = 0x0;
	*FTMx_C0V[FTM] = channelValue;
}
...
void FTM_Init(const FTM_ConfigType * FTM_Config)
{
	...
	FTM_C0SC(FTM_Config->FTM_Channel, FTM_Config->FTM_C0SC_Mask);
