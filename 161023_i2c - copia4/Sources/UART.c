...
void UART_init(UART_ChannelType uartChannel, uint32 systemClk, UART_BaudRateType baudRate){
	double UARTbdrt = ((double)systemClk)/(16*((double)baudRate));
	uint8 UARTbrfd = (((double)systemClk)/(16*((double)baudRate))-UARTbdrt)*32.0;
	UARTx_clockEnable(uartChannel);
	UARTx_interruptEnable(uartChannel);
	PORTB_PCR(16) = PORT_PCR_MUX(3);  //cambiar
	PORTB_PCR(17) = PORT_PCR_MUX(3);  //cambiar
	*UARTx_BDL[uartChannel] = ((uint32)UARTbdrt) & 0xFF;
	*UARTx_BDH[uartChannel] = (( ((uint32)UARTbdrt) >>8)&(0x1F));
